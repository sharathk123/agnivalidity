...
export const SourceCard: React.FC<SourceCardProps> = ({ source, onRun, onStop, disabled }) => {
    ...
    return (
        <div className={`bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-700/80 rounded-lg p-6 group transition-all duration-300 hover:shadow-[0_0_30px_rgba(99,102,241,0.15)] dark:hover:shadow-[0_0_30px_rgba(99,102,241,0.1)] hover:border-indigo-400/50 dark:hover:border-indigo-500/50 hover:scale-[1.01] relative overflow-hidden ${!source.is_active ? 'opacity-60 grayscale' : ''}`}>
            ...
            <div className="grid grid-cols-2 gap-4 pb-4 border-b border-slate-200 dark:border-slate-800 mb-4 relative z-10">
                <div>
                    <div className="text-[9px] font-black text-slate-500 dark:text-slate-500 uppercase tracking-widest mb-1">Frequency</div>
                    <div className="text-[11px] font-bold text-slate-700 dark:text-slate-300 font-mono">{source.frequency}</div>
                </div>
                <div className="text-right">
                    <div className="text-[9px] font-black text-slate-500 dark:text-slate-500 uppercase tracking-widest mb-1">Records Tracking</div>
            ...
            <div className={`flex justify-between items-center bg-slate-50 dark:bg-slate-950/50 -mx-6 -mb-6 px-6 py-3 border-t border-slate-200 dark:border-slate-800 mt-2 relative z-10`}>
                <div className="flex flex-col">
                    <span className="text-[8px] font-black text-slate-500 dark:text-slate-600 uppercase tracking-widest">Last Run Registry</span>
                ...
                <div className={`px-2 py-0.5 rounded text-[9px] font-black uppercase tracking-widest ${source.last_run_status === 'SUCCESS' ? 'bg-emerald-100 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-500/20' :
                    source.last_run_status === 'FAILED' ? 'bg-rose-100 dark:bg-rose-500/10 text-rose-600 dark:text-rose-400 border border-rose-200 dark:border-rose-500/20' :
                        isRunning ? 'bg-indigo-100 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border border-indigo-200 dark:border-indigo-500/20 animate-pulse' :
