        ...
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700/50 p-6 rounded-lg shadow-sm dark:shadow-[0_0_15px_rgba(79,70,229,0.05)] transition-colors duration-300">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
                ...
                <div className="bg-slate-50 dark:bg-slate-950/50 border border-slate-200 dark:border-slate-800 rounded p-4 relative overflow-hidden group">
                    ...
                    <div className="text-right text-[9px] font-mono text-emerald-600 dark:text-emerald-400 mt-2 font-bold">+4.5% Aggregate</div>
                </div>
            </div>
        </div>
    );
...
const MetricCard = ({ label, value, subtext, icon }: any) => (
    <div className="bg-slate-50 dark:bg-slate-950/50 border border-slate-200 dark:border-slate-800 rounded p-4 hover:border-indigo-400 dark:hover:border-indigo-500/30 transition-all group">
        <div className="flex justify-between items-start mb-4">
            <div className="p-2 bg-white dark:bg-slate-900 rounded border border-slate-200 dark:border-slate-800 group-hover:border-indigo-300 dark:group-hover:border-indigo-500/30 transition-colors shadow-sm dark:shadow-none">
                {icon}
            </div>
            <div className="text-lg font-bold text-emerald-600 dark:text-emerald-400 font-mono tracking-tight">{value}</div>
        </div>
        <div>
            <div className="text-[10px] font-black text-slate-700 dark:text-slate-200 uppercase tracking-widest truncate">{label}</div>
