        ...
        <div className="p-6 h-screen flex flex-col gap-6 animate-fade-in overflow-hidden relative">
            ...
            <div className="shrink-0">
                <div className="flex justify-between items-end mb-4">
                    <div>
                        <h2 className="text-3xl font-black font-display text-white tracking-tighter uppercase">Market Intelligence</h2>
                        <div className="text-[10px] font-mono text-slate-500 uppercase tracking-widest mt-1">Consolidated Surveillance & Analytics Hub</div>
                    </div>
                </div>
                <GlobalMarketPulse />
            </div>
            ...
            <div className="flex-1 min-h-0 flex gap-6">
                ...
                <div className="w-[30%] bg-slate-950/50 border border-slate-800 rounded-lg p-4 flex flex-col">
                    <div className="flex items-center gap-2 mb-4">
                        <Zap className="w-4 h-4 text-emerald-400" />
                        <h3 className="text-xs font-black text-slate-300 uppercase tracking-widest font-display">Live Signals</h3>
                    </div>

                    {/* Seasonality Signal (Moved from Chart Overlay) */}
                    <div className="mb-4 rounded-lg overflow-hidden border border-slate-800/50">
                        <SeasonalitySignalCard />
                    </div>

                    <div className="flex-1 overflow-y-auto pr-2 space-y-2 custom-scrollbar">
                        {signals.map(signal => (
                            <div key={signal.id} className="p-3 bg-slate-900 border-l-2 border-emerald-500/50 hover:border-emerald-400 hover:bg-slate-800 transition-all group cursor-pointer">
                                ...
                                <p className="text-[10px] font-mono text-emerald-100/80 leading-relaxed group-hover:text-white transition-colors">
                                    {signal.text}
                                </p>
                            </div>
                        ))}
                ...
                <div className="w-[70%] bg-slate-950/50 border border-slate-800 rounded-lg p-4 flex flex-col relative group">
                    <div className="flex justify-between items-start mb-4">
                        <div className="flex items-center gap-2">
                            <Activity className="w-4 h-4 text-indigo-400" />
                            <div>
                                <h3 className="text-xs font-black text-slate-300 uppercase tracking-widest font-display">Trend Engine</h3>
                                <div className="text-[9px] font-mono text-slate-600 uppercase">Real-time Analytics</div>
                            </div>
                        </div>

                        {/* Chart Toggle */}
                        <div className="flex bg-slate-900 border border-slate-700/50 rounded p-0.5">
                            <button onClick={() => setViewMode('PRICE')} className={`px-3 py-1 rounded text-[9px] font-black uppercase tracking-widest transition-all ${viewMode === 'PRICE' ? 'bg-indigo-600 text-white' : 'text-slate-500 hover:text-slate-300'}`}>Price</button>
                            <button onClick={() => setViewMode('PROFIT')} className={`px-3 py-1 rounded text-[9px] font-black uppercase tracking-widest transition-all ${viewMode === 'PROFIT' ? 'bg-emerald-600 text-white' : 'text-slate-500 hover:text-slate-300'}`}>Profit</button>
                        </div>
                    </div>

                    <div className="flex-1 relative bg-slate-900/20 rounded border border-slate-700/30">
                        {/* <SeasonalitySignalCard /> Moved to Left Panel */}
                        <ResponsiveContainer width="100%" height="100%">
                            <AreaChart data={chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                                ...
                                <XAxis dataKey="name" stroke="#475569" tick={{ fontSize: 10 }} axisLine={false} tickLine={false} dy={10} />
            ...
            {/* 3. Telemetry Stream (Collapsible Footer Drawer) */}
